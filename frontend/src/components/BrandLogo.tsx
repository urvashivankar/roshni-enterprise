...
const BrandLogo: React.FC<BrandLogoProps> = ({ className = '', size = 'md', showText = true, variant = 'dark' }) => {
    ...
    return (
        <div className={`flex items-center ${currentSize.gap} ${className}`}>
            ...
            <div className={`${currentSize.icon} bg-blue-600 rounded-xl flex items-center justify-center shadow-lg transform rotate-[-5deg] hover:rotate-0 transition-transform duration-300`}>
                <Snowflake className="w-2/3 h-2/3 text-white animate-pulse-slow" />
            </div>
